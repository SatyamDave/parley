...
- **`parley_update` still self-attests.** Reading `src/mcp.ts` as of this writing, `verified_by` and `verified_how` are set directly from whatever the calling agent passes — nothing is actually run to confirm it. `src/checks.ts` exists and can genuinely run a project's real checks (npm/pnpm/yarn/bun scripts, pytest/ruff/mypy, `go test`/`vet`/`build`, `cargo test`/`clippy`/`build`, a Makefile's `test`/`check` target) and report the real exit code and output — but nothing in the codebase calls it yet; it isn't wired into `parley_update` or anywhere else. `src/checks.ts`'s own header describes exactly why this matters: an agent once marked 49 real tests done on the strength of its own claim, while the test script itself had never actually run. (This file is being actively worked on elsewhere; re-read `src/mcp.ts` if this has since changed.)
